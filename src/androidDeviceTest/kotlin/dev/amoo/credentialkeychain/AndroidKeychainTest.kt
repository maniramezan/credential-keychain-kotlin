...
class AndroidKeychainTest {
    ...
    @Test fun roundTripPersistsAndIsolatesAccounts() = withStore { service, first ->
        ...
        } finally { second.delete("key"); removeNamespace(service, "second") }
    }

    @Test fun tamperingAndTruncationFailWithoutReturningPlaintext() = withStore { service, store ->
    ...
    @Test fun ciphertextCannotBeMovedBetweenKeys() = withStore { service, store ->
    ...
    @Test fun missingEncryptionKeyIsNeverSilentlyReplaced() = withStore { service, store ->
        store.write("key", "secret")
    ...
    @Test fun writesUseFreshIvsAndNeverPersistPlaintext() = withStore { service, store ->
    ...
    @Test fun concurrentInstancesKeepIndependentEntriesIntact() = withStore { service, _ ->
        val executor = Executors.newFixedThreadPool(4)
        try {
            val futures = (0 until 12).map { index -> executor.submit {
    ...
    @Test fun legacyAtomicBackupIsRecovered() = withStore { service, store ->
        store.write("key", "secret")
        val file = file(service, "key")
    ...
    private fun alias(service: String, account: String = "first") =
        "dev.amoo.credentialkeychain.${digest(credentialNamespace(service, account))}"
    private fun file(service: String, key: String) = File(File(context.noBackupFilesDir, alias(service)), "${digest(key)}.bin")
    private fun digest(value: String) = MessageDigest.getInstance("SHA-256")
        .digest(value.toByteArray()).joinToString("") { "%02x".format(it) }
    private fun removeNamespace(service: String, account: String = "first") {
        File(context.noBackupFilesDir, alias(service, account)).deleteRecursively()
        KeyStore.getInstance("AndroidKeyStore").apply { load(null); deleteEntry(alias(service, account)) }
    }
    private fun withStore(block: (String, CredentialKeychain) -> Unit) {
        val service = "keychain-test-${UUID.randomUUID()}"
        try { block(service, create(service)) } finally { removeNamespace(service) }
    }
}
