...
internal fun findExecutable(name: String): java.io.File? = System.getenv("PATH")
...
internal fun runCommand(vararg command: String, stdin: String? = null, timeoutSeconds: Long = 30): CommandResult {
    val executor = java.util.concurrent.Executors.newFixedThreadPool(3) { runnable ->
    ...
    try {
        ...
        return CommandResult(child.exitValue(), stdout.get(remaining(), java.util.concurrent.TimeUnit.NANOSECONDS),
...
internal fun interface CommandRunner {
    fun run(arguments: List<String>, stdin: String?): CommandResult
}

internal val systemCommandRunner = CommandRunner { arguments, stdin ->
